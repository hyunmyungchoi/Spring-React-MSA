...
| AWS migration | Runtime OFF·RDS 정지 유지; 관측성 첫 Apply 부분 실패 교정 후 복구 Plan `6/0/0` 승인 대기 |
...
| [Learning Runtime 결정](aws-migration/07-learning-runtime-design.md) | NAT, State, ECS, RDS, Frontend, Secret, DNS 결정 | Runtime ON HTTPS·OAuth·Session·WebSocket 검증과 후속 OFF 완료; 관측성 Policy 복구 Apply 대기 |
