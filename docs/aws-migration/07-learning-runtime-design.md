...
현재 Terraform은 `enable_ecs_compute_foundation = true`, `learning_runtime_enabled = false`로 Compute 기반과 유료 실행 용량을 분리한다. 첫 저장 Plan은 현재 App Subnet AZ가 `2a·2b`이고 `m5a.xlarge`는 `2a·2c`에만 제공되어 폐기했다. 기존 Network/Data Layer를 교체하지 않고 두 App AZ 모두에서 제공되는 `m6i.xlarge`로 변경했다. 새 Plan은 ECS Cluster, Launch Template, ASG, Capacity Provider, EC2 Instance Role/Profile 등 9개 추가, 기존 리소스 변경·삭제 없음과 ASG `0/0/0`을 확인한 뒤 Apply했다. Cluster와 Capacity Provider `ACTIVE`, EC2/Task/Service 0개와 재계획 `No changes`를 검증했다. Backend Task Definition과 Service는 후속 Plan에서 다룬다.
...
`learning_runtime_enabled`는 ECS/ASG/ALB/NAT 같은 실행 비용 리소스의 목표 상태를 제어한다. RDS 정지는 Terraform 리소스 삭제와 다르므로 별도 운영 명령 또는 자동화로 수행한다.
...
이 결정은 코드에 아직 적용되지 않았다. 공개 관리자 가입 차단과 원본 Session ID 제거가 완료되기 전에는 AWS Public Traffic을 열지 않는다.
...
상위 아키텍처 결정과 Compute 구현값은 확정했다. Task/Service와 후속 Runtime 구현 전에 다음 값은 추가로 확정해야 한다.

- RDS 구현값은 확정 완료: PostgreSQL 16.14, `db.t4g.micro`, Single-AZ, 20 GiB `gp3`, `postgres16` Parameter Group, 삭제 보호, UTC 일요일 18:00 유지보수 시간
- Redis를 ElastiCache Redis OSS 또는 Valkey 중 무엇으로 구성할지와 Node 크기
- ECS 내부 통신을 Cloud Map 또는 Service Connect 중 무엇으로 구성할지
- 서비스별 Task Role/Execution Role의 정확한 IAM 권한
- ALB Listener Rule, Health Check Path와 CloudFront Origin 검증 방식
...
이 값들은 승인된 큰 구조를 바꾸지 않는 구현 세부사항이지만, 첫 Runtime Plan 전에 결정하고 테스트로 고정해야 한다.
