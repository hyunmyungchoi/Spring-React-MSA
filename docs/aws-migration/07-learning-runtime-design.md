...
> 문서 상태: Hikari `5/1` 재측정과 RDS Alarm 5개·Member BFF Prometheus 200/404 Runtime ON/OFF 전체 검증, RDS 정지·Applied Plan/로그 정리·최종 감사 완료
>
> 기준일: 2026-07-24
>
...
>
> AWS 적용 상태: Runtime ON 전체 Smoke 뒤 OFF Plan `0/10/40` 적용과 최종 정리를 완료했다. 현재 State serial 131·주소 251개, ECS/Task/Container Instance·ASG Instance·ALB·Valkey·`origin`·Runtime Alarm은 모두 0이고 Container Insights는 disabled다. RDS는 stopped, 영속 RDS Alarm 5개·Watchdog Alarm 3개와 SNS·Task Definition/Image 8개·Frontend를 유지한다. Applied Saved Plan과 Runtime Apply 로그 잔여는 0이다.
...
18. 완료: Hikari 재측정 Runtime OFF Saved Plan `0/10/40` 적용, Runtime 리소스 0 수렴·RDS 정지·State serial 119·OFF `No changes`·정적 curl 6/6·Root 308·API 502 검증
19. 완료: `db.t4g.micro` 유지, FreeableMemory 128 MiB·SwapUsage 64 MiB·DatabaseConnections 16 Alarm과 Member BFF Prometheus 200/404 후속 결정
...
26. 완료: Runtime OFF 사전 점검과 Saved Plan `0/10/40` 생성, 삭제 40개 Runtime 경계·RDS/Task/Image/Secret/Network/Frontend 변경 0 검증
27. 완료: 검증된 Runtime OFF Saved Plan `0/10/40` 적용, State serial 131·주소 251개, Runtime 0·정적 curl·OFF `No changes`·RDS 정지
28. 완료: Applied Saved Plan 4개·Runtime Apply 로그 4개 삭제, 최종 OFF `No changes`·AWS/Git/문서·curl 9/9 감사
...
- 완료: Runtime OFF Saved Plan `0/10/40` 적용, ECS·ASG·ALB·Valkey·Runtime Alarm·`origin` 0, RDS `stopped`, State serial 119·동일 OFF 입력 `No changes`
- 완료: `db.t4g.micro` 유지와 FreeableMemory 128 MiB·SwapUsage 64 MiB·DatabaseConnections 16 영속 Alarm 결정
- 완료: Member BFF Prometheus Registry 누락·미존재 Resource 500 원인 진단과 200/404 교정 범위 결정
...
- 완료: Runtime OFF 사전 점검과 Saved Plan `0/10/40` 생성, Hash·비용·삭제 경계와 AWS 무변경 검증
- 완료: 검증된 Runtime OFF Saved Plan `0/10/40` 적용, Runtime 0·정적 curl·OFF `No changes`와 RDS 정지
- 완료: Applied Saved Plan 4개·Runtime Apply 로그 4개 삭제, 최종 OFF `No changes`·AWS/Git/문서·curl 9/9 감사
