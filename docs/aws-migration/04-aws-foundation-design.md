...
> 기준일: 2026-07-19
>
> 저장소 상태: Network/ECR/OIDC/Private App 송신·Data·Compute·Application Foundation Terraform 코드 존재
>
> AWS 적용 상태: Foundation·ECR/OIDC·Private App 송신·Data·Compute·Application Foundation 유지, Runtime ON Smoke 검증 후 현재 ECS/ASG 0·ALB/Valkey 삭제·RDS 정지
...
1. 인터넷 요청은 두 Public Subnet의 ALB까지 도달한다.
...
5. Runtime ON 검증에서 두 Host Rule, Gateway Target 2/2, 내부 Authorization/BFF 경로와 curl Smoke 6/6을 확인했다.
...
1. 완료: Secrets Manager Container와 RDS PostgreSQL 16 Terraform을 AWS에 적용하고 실제 RDS Role·Schema Bootstrap·Flyway V1까지 검증했다.
2. 완료: ECS on EC2 Cluster와 ASG Capacity Provider, Digest 고정 Application Task Definition·Service 8개와 Cloud Map을 적용하고 Runtime OFF 수렴을 검증했다.
3. 완료: Runtime ON으로 Public ALB·Valkey·Task 8개를 실행하고 Health·Digest·Cloud Map, 내부 호출과 curl Smoke 6/6을 검증했다.
4. 완료: Runtime OFF Saved Plan을 적용하고 ECS/ASG 0, Valkey·ALB 삭제, RDS 정지와 재계획 `No changes`를 검증했다.
5. Frontend S3/CloudFront, ACM과 Route 53 Record를 구현한다.
6. Learning ON/OFF 자동화, CloudWatch Logs/Metrics/Alarms와 Backup Restore 절차를 검증한다.
7. 완료: Build Once·Digest Promote Workflow로 ECR 재빌드 방식을 교체하고 Migration Image 3개와 Application Backend 8개를 실제 검증했다.
