...
1. NAT Gateway와 VPC Endpoint 비용 및 장애 범위를 비교한다.
2. ECR, ECS Cluster, EC2 Auto Scaling Group, Capacity Provider를 설계한다.
3. ALB, Target Group, Listener, ACM, Route 53을 설계한다.
4. RDS/ElastiCache 도입 전에 검증된 DDL 또는 마이그레이션 체계를 확정한다.
5. CloudWatch Logs, Metrics, Alarms와 운영 Runbook을 작성한다.
