...
> 기준일: 2026-07-23
>
> 현재 상태: Hikari `5/1` 재측정 Runtime ON과 30분 전체 Smoke·후속 Runtime OFF 완료; 원본 RDS `stopped`, ECS·ASG·ALB·Valkey·Runtime Alarm 0, State serial 119·OFF `No changes`
>
...
Pool 교정은 성공했지만 FreeableMemory 30/30점이 256MiB 아래여서 Runtime ON의 Alarm은 실제 `ALARM`이었다. Alarm만 낮추지 않고 `db.t4g.small`과 기준선 기반 Alarm 재설계를 별도 결정으로 함께 검토한다. Member BFF `/actuator/prometheus`의 500 `INTERNAL_SERVER_ERROR`도 별도 진단 대상으로 남긴다. Runtime OFF·RDS 정지는 완료했으며 상세 실측과 후속 결정은 [RDS 메모리·연결 풀 교정 계획](../plans/2026-07-23-rds-memory-plan.md)을 따른다.

Runtime OFF Saved Plan `tfplan-rds-hikari-runtime-off`은 State serial 113 기준 `0 add, 10 change, 40 destroy`, SHA-256 `5e3f9b9a03dceab9eb57491b57b05a8c090693c2c41c10f047ee2c9b86cd779d`였다. 승인된 파일을 그대로 적용해 ECS·ASG·ALB·Valkey·Runtime Alarm·`origin`을 0으로 만들고 원본 RDS를 `2026-07-24 02:40:45 KST`에 정지했다. 자동 재시작 예정은 `2026-07-31 02:40:28.880 KST`다. 최종 State serial 119·주소 249개, 동일 OFF 입력 `No changes`이며 Task Definition·Secret·Frontend·Watchdog·감사 Log는 보존했다.

## 10. 재측정 후 Class·Alarm 운영 기준

2026-07-24 결정은 `db.t4g.micro` 유지다. FreeableMemory 최소 190.14 MiB에서도 Swap 최대 0.45 MiB, CPU 평균 4.07%, DatabaseConnections 최대 6으로 안정적이었으므로 비용이 약 2.04배인 `db.t4g.small`로 즉시 전환하지 않는다.

후속 Terraform 목표는 FreeableMemory 128 MiB, SwapUsage 64 MiB, DatabaseConnections 16과 기존 CPU 80%·FreeStorage 5 GiB를 합친 영속 Alarm 5개다. 평가 계약은 5분 3/3, RDS 정지 중에는 `treat_missing_data=notBreaching`이다. 현재 AWS 적용값은 아직 기존 Alarm 3개와 FreeableMemory 256 MiB이므로 구현·Foundation Saved Plan 적용 전에는 목표값으로 판정하지 않는다.

FreeableMemory 128 MiB 이하 또는 SwapUsage 64 MiB 이상이 지속되거나 OOM·DB 재시작이 확인되면 별도 승인으로 `db.t4g.small` 전환을 검토한다. Class 변경을 Alarm·Member BFF 교정 Plan에 섞지 않는다. 상세 근거와 다음 승인 Gate는 [RDS Alarm·Member BFF Prometheus 교정 계획](../plans/2026-07-24-rds-alarm-prometheus-plan.md)을 따른다.
