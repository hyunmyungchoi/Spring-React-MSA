...
> 실행 상태(2026-07-24): Hikari `5/1` 재측정 Runtime ON과 30분 전체 Smoke 뒤 승인된 Runtime OFF Saved Plan SHA-256 `5e3f9b9a03dceab9eb57491b57b05a8c090693c2c41c10f047ee2c9b86cd779d`을 `0/10/40`으로 적용했다. 현재 ECS Service 8개 `0/0/0`, ASG `0/0/0`, EC2·Task·Container Instance·ALB·Valkey·Runtime Alarm·`origin`은 0, RDS는 `stopped`, State serial 119이며 동일 OFF 입력은 `No changes`다.
...
> Hikari 재측정 Runtime OFF Plan 준비 완료(Apply 전): `tfplan-rds-hikari-runtime-off`은 251,142 bytes, SHA-256 `5e3f9b9a03dceab9eb57491b57b05a8c090693c2c41c10f047ee2c9b86cd779d`, State serial 113 기준 `0 add, 10 change, 40 destroy`다. ECS Service 8개, ASG Min/Max, Container Insights를 OFF로 바꾸고 Runtime Alarm 29개·Valkey 6개·ALB 계열 4개·`origin` 1개만 삭제한다. RDS·Task Definition·Secret·Frontend·Watchdog·Restore 감사 Log 변경과 교체는 0이며 Redis Password는 직렬화되지 않았다. Gate 만료는 `2026-07-24 03:35:38.853 KST`다. RDS 정지는 Apply 수렴 뒤 별도 API로 수행한다.
...
최신 승인 Plan `tfplan-rds-hikari-runtime-off`, SHA-256 `5e3f9b9a03dceab9eb57491b57b05a8c090693c2c41c10f047ee2c9b86cd779d`은 State serial 113 기준 `0 added, 10 changed, 40 destroyed`로 적용했다. ECS·ASG·ALB·Valkey·Runtime Alarm·`origin`을 0으로 수렴시켰으며 RDS·Restore 감사 Log·Task Definition·Frontend·Secret·SNS Subscription은 변경하지 않았다. 원본 RDS는 `2026-07-24 02:40:45 KST`에 `stopped`가 됐고 자동 재시작 예정은 `2026-07-31 02:40:28.880 KST`다. State serial 119의 OFF 재계획은 `No changes`, 정적 curl 6/6은 200, Root는 308, Member/Admin API는 502다. 적용 Plan은 Hash 재검증 뒤 삭제한다.
