...
**AWS Learning 결정:** 현재 구현의 Endpoint는 공개지만 AWS Public Traffic에서는 활성화하지 않는다. 최초 관리자는 일회성 Bootstrap 절차로 생성하고 이후 Registration Route를 닫는다. 정확한 Bootstrap 실행 주체와 감사 방식은 구현 전에 확정한다. 이 차단이 코드와 배포 설정에서 검증되기 전에는 AWS Public Traffic을 열지 않는다.
...
현재 세션 목록은 원본 `sessionId`를 반환하므로 아직 목표 보안 계약을 만족하지 않는다. 목표 응답은 원본 값을 제거하고 SHA-256 Fingerprint 또는 마스킹된 식별자만 반환한다. 원본 Session ID가 필요한 강제 Logout은 별도 인증된 POST Endpoint 내부에서만 처리하고 감사 로그를 남긴다.
