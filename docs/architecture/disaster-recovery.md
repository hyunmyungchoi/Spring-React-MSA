...
이 문서는 현재 실행 가능한 기능을 설명하지 않는다. 2026-07-18 범위 결정으로 Kubernetes↔AWS DR은 Learning 적용 대상에서 제외하고 후속 학습 과제로 보류했다. 저장소에는 로컬 Kubernetes 배포 경로와 일부 AWS Foundation/ECR 준비만 있고, AWS Workload와 교차 환경 Data Replication은 아직 없다.
...
1. AWS RDS/Secrets와 DB Bootstrap/Flyway는 적용·검증했지만 RDS는 정지 상태다. ECS/ALB/Valkey Application Runtime 코드는 준비됐으나 아직 AWS workload는 없다.
