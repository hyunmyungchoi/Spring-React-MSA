...
- 상태: 운영 환경 참고 아키텍처, Learning 범위 제외·미구현
- 기준일: 2026-07-19
...
이 문서는 현재 실행 가능한 기능을 설명하지 않는다. 2026-07-18 범위 결정으로 Kubernetes↔AWS DR은 Learning 적용 대상에서 제외하고 후속 학습 과제로 보류했다. AWS Learning Runtime의 RDS·Valkey·Public ALB와 Digest 고정 ECS Service 8개는 기동·Smoke 검증했지만 교차 환경 Data Replication, Traffic 전환과 단일 Writer Fencing은 없다.

Learning의 실제 복구 범위는 RDS Automated Backup 7일, PITR, 의도적인 삭제 전 Final Snapshot, Terraform Backend Versioning과 Restore 훈련으로 제한한다. 아래 Warm Standby 구조와 수치는 운영 환경 비교를 위한 목표이며 현재 프로젝트의 AWS 적용 계획이나 보장값이 아니다.
...
| AWS | Foundation·ECR/OIDC·RDS·ECS/ALB/Valkey 적용과 Backend 8개 Runtime ON 검증 완료 | 교차 환경 복제와 전환 통제를 갖춘 warm standby |
...
1. AWS Learning Runtime 자체는 RDS·Valkey·Public ALB와 ECS Service 8개까지 기동·검증했지만 Kubernetes와 연결된 warm standby나 자동 전환 대상은 아니다.
