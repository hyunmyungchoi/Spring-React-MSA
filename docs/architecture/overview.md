...
현재 동작이 저장소에서 검증되는 배포 경로는 GHCR 이미지와 로컬 Kubernetes/Argo CD다. AWS에는 Terraform Foundation이 적용됐다고 운영 문서에 기록되어 있고 ECR/OIDC 코드는 준비됐지만, ECS·ALB·RDS·ElastiCache workload는 아직 없다.

AWS 전환 자료는 [`docs/aws-migration`](../aws-migration/00-service-inventory.md)에 두고 현재 서비스 계약과 중복 작성하지 않는다. Kubernetes를 선호 site, AWS를 warm standby로 사용하는 양방향 Active-Passive DR 목표는 [재해 복구 아키텍처](disaster-recovery.md)를 따른다. 이 DR은 아직 제안·계획 상태이며 현재 실행 가능한 기능이 아니다.
