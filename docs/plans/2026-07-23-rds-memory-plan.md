...
## 10. DB Class·Alarm·Prometheus 후속 결정

2026-07-24 별도 진단 결과 Hikari `5/1` 재측정은 `DatabaseConnections` 평균 3.87·최대 6, `FreeableMemory` 최소 190.14 MiB, `SwapUsage` 최대 0.45 MiB, CPU 평균 4.07%였다. 연결 수가 정상화되고 Swap·CPU가 낮으므로 `db.t4g.micro`를 유지한다.

기존 FreeableMemory 256 MiB 단독 기준은 `db.t4g.micro`의 안정 기준선과 맞지 않아 128 MiB로 조정하고, SwapUsage 64 MiB와 DatabaseConnections 16 Alarm을 추가한다. CPU·FreeStorage를 포함한 영속 RDS Alarm 목표는 5개다. Member BFF `/actuator/prometheus` 500은 Prometheus Registry 누락과 미존재 Resource catch-all 500 변환이 원인이다.

이 결정은 아직 코드나 AWS에 적용되지 않았다. 구현·테스트·Image Promote·Saved Plan 범위와 재승인 조건은 [RDS Alarm·Member BFF Prometheus 교정 계획](2026-07-24-rds-alarm-prometheus-plan.md)을 따른다.
