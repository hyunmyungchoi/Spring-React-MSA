...
2B는 15분마다 실행되는 Python 3.12/ARM64 Lambda, EventBridge Schedule, 중복 알림 방지용 DynamoDB On-Demand Table, Heartbeat Custom Metric과 Watchdog 자체 Alarm 3개로 구현·적용했다. Runtime 6시간 초과, RDS 자동 재시작 24시간 이내, Runtime OFF 중 RDS 실행을 감지하고 상태가 바뀔 때만 기존 SNS Topic으로 ALERT/RECOVERY를 발행한다. IAM은 RDS·ASG·EC2·ECS Describe만 허용하며 Runtime 시작·정지·용량 변경 권한은 없다. Python 단위 테스트 `7 passed`, 전체 Terraform mock 테스트 `30 passed, 0 failed`, `validate`를 통과했다.

최초 Plan SHA-256 `b1c71772b203ae9fd671af58940dc9a0ace49c978a60ca33856225ad51121d9b` 적용은 계정 동시성 한도 10에서 `reserved concurrency=1`이 AWS의 최소 미예약 동시성 10 조건을 위반해 Lambda 생성 뒤 중단됐다. 예약 동시성을 제거한 Source SHA `2f9566bfee08504153a2775b11ac554ceffe53cc`에서 부분 state를 안전하게 수렴시킨 Recovery Plan은 213,517 bytes, SHA-256 `1ae5c62eced0fe89b718a8a005f555d3c2287c5fd792959abb5b6394d8dfcf35`, `2 create, 0 update, 0 destroy`였고 승인 적용 결과도 `2 added, 0 changed, 0 destroyed`였다. Baseline Invoke에서 Runtime inactive·ASG/ECS/Instance 0·RDS `stopped`·이슈 false 3개·전환 0, DynamoDB inactive 3개, Heartbeat `1.0`을 확인했다. 승인된 Heartbeat Alarm OK 복구는 SNS Action 성공과 `Delivered 1`로 검증했고 최종 Watchdog Alarm 3개 모두 `OK`, Email Subscription `Confirmed`, 동일 OFF 입력 `No changes`다. 이로써 2B는 완료됐으며 다음 단계는 최초 관리자 Bootstrap이다.
