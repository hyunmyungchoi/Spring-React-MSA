...
public class BffTokenService {
    ...
    public BffTokenService(RestClient.Builder restClientBuilder) {
    ...
    public String getAccessTokenOrThrow(HttpSession session) {
        ...
        if (accessToken == null || accessToken.isBlank()) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "BFF session has no access token");
        }
    ...
    public OAuth2TokenResponse refreshAccessToken(HttpSession session) {
        ...
        if (refreshToken == null || refreshToken.isBlank()) {
            session.invalidate();
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "BFF session has no refresh token");
        }
