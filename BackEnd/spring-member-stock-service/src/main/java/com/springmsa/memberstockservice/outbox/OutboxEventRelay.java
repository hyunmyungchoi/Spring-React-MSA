package com.springmsa.memberstockservice.outbox;
...
public class OutboxEventRelay {
    ...
    public void publishPending() {
        List<PendingEvent> events = jdbcTemplate.query("""
                        SELECT event_id, topic, event_key, payload FROM stock_service.outbox_events
        ...
        for (PendingEvent event : events) {
            try {
                ...
                jdbcTemplate.update(
                        "UPDATE stock_service.outbox_events SET published_at = ?, attempts = attempts + 1, last_error = NULL WHERE event_id = ?",
                        OffsetDateTime.now(ZoneOffset.UTC), event.eventId());
            } catch (Exception exception) {
                String message = exception.getMessage() == null ? exception.getClass().getSimpleName() : exception.getMessage();
                jdbcTemplate.update(
                        "UPDATE stock_service.outbox_events SET attempts = attempts + 1, last_error = ? WHERE event_id = ?",
