package com.springmsa.adminbff.auth.controller;
...
public class AdminBffAuthController {
    ...
    @GetMapping("/auth/me")
    public ResponseEntity<MsaResponse<AdminAuthMeResponse>> me(Authentication authentication, CsrfToken csrfToken) {
        ...
        if (!adminBffAuthenticationService.isAuthenticated(authentication)) {
            return ResponseEntity.ok(MsaResponse.ok(AdminAuthMeResponse.anonymous()));
        }

        if (!adminBffAuthenticationService.hasAdminRole(authentication)) {
            return ResponseEntity
                    .status(HttpStatus.FORBIDDEN)
                    .body(MsaResponse.fail(HttpStatus.FORBIDDEN, "ADMIN_ROLE_REQUIRED", "Admin role is required", AdminAuthMeResponse.adminRoleRequired(), java.util.List.of()));
        }

        return ResponseEntity.ok(
                MsaResponse.ok(AdminAuthMeResponse.authenticated(
    ...
    public ResponseEntity<MsaResponse<AdminLogoutResponse>> logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (!StringUtils.hasText(idToken)) {
            return ResponseEntity.ok(MsaResponse.ok(AdminLogoutResponse.localSuccess()));
        }

        String authServerLogoutUrl = endSessionUrl(idToken, postLogoutRedirectUri);

        return ResponseEntity.ok(MsaResponse.ok(AdminLogoutResponse.success(authServerLogoutUrl)));
    }
