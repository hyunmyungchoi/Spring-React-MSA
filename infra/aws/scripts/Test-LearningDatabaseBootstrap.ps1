...
if ($taskResult.failures.Count -gt 0 -or $taskResult.tasks.Count -ne 1) {
    throw "ECS did not start exactly one verification task."
}

$taskArn = [string]$taskResult.tasks[0].taskArn
$taskId = $taskArn.Split("/")[-1]
Write-Host "Verification task started: $taskArn"
...
if ($exitCode -ne 0) {
    throw "Database verification task failed with exit code $exitCode."
}

$logStream = "bootstrap/db-bootstrap/$taskId"
$messages = @()
for ($attempt = 0; $attempt -lt 10 -and $messages.Count -lt 5; $attempt++) {
...
foreach ($expectedMessage in $expectedMessages) {
    if ($messages -notcontains $expectedMessage) {
        throw "Missing verification result: $expectedMessage"
    }
}

$messages | Where-Object { $_ -in $expectedMessages } | ForEach-Object { Write-Host $_ }
Write-Host "Database bootstrap verification completed with exit code 0."
