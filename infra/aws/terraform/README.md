...
이 디렉터리는 `spring-react-msa` 학습 환경의 현재 AWS 인프라를 관리한다. 기본 리전은 `ap-northeast-2`다. 현재 구성은 이미 적용된 Foundation 기준선과, 생성·검토가 끝난 저장 Plan으로 아직 Apply하지 않은 Backend ECR/GitHub OIDC 확장으로 나뉜다. ECS 등의 실제 워크로드는 아직 배포하지 않는다.
...
### 현재 구성에 포함됐지만 아직 Apply하지 않은 ECR/OIDC 확장

검토된 `infra/aws/terraform/tfplan.ecr`은 다음 18개 리소스만 추가한다.
...
- 검토된 Plan Apply: 아직 실행하지 않음
- GitHub Repository Variable `AWS_ECR_PUSH_ROLE_ARN` 등록: 아직 하지 않음
...
아래 단계는 아직 실행하지 않았다. 검토된 `tfplan.ecr` Apply가 성공한 뒤 Role ARN을 GitHub Repository Variable로 등록한다.
...
1. 검토된 `tfplan.ecr`의 명시적 Apply 승인과 실행
2. `AWS_ECR_PUSH_ROLE_ARN` 등록과 단일 Backend Image 게시 검증
