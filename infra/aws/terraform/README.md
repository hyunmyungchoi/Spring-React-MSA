...
이 디렉터리는 `spring-react-msa` 학습 환경의 현재 AWS 인프라를 관리한다. 기본 리전은 `ap-northeast-2`다. Foundation 기준선, Backend ECR/GitHub OIDC, Private App 송신, RDS/Secrets Data Layer, ECS Compute, Database Bootstrap·Flyway Migration, Application·Frontend·Public Domain Foundation과 Runtime 관측성 수명주기를 Apply했다. 2026-07-22 Runtime ON HTTPS·OAuth·Session·WebSocket·SNS 검증 뒤 승인된 OFF Plan으로 현재 ECS Service·Task·Container Instance·ASG는 모두 0, Public ALB·Valkey·`origin`·Runtime Alarm은 0, Container Insights는 `disabled`, RDS는 `stopped`다. Frontend S3 6개·CloudFront 2개, Digest 고정 Task Definition 8개, 확인된 SNS Email Subscription과 RDS Alarm 3개는 유지한다. 정적 curl 6/6 HTTP 200, Root 308, Runtime OFF API 502와 동일 OFF 입력 Terraform `No changes`를 확인했다. 후속 Runtime의 승인된 목표와 미구현 경계는 [AWS Learning Runtime 결정](../../../docs/aws-migration/07-learning-runtime-design.md)을 따른다.
...
> 2026-07-22 갱신: SNS 실알림 검증과 Runtime OFF 비용 종료까지 완료했다. 현재 실행 Workload·ALB·Valkey·Runtime Alarm은 0, RDS는 `stopped`, Terraform은 `No changes`다. 알림 전용 Watchdog 코드는 구현·테스트를 마쳤고 AWS Saved Plan 승인 대기 상태다. 이후 최초 관리자 Bootstrap과 Backup Restore 순서로 진행한다.
...
10. 진행 중: Runtime OFF 알림 전용 Watchdog 코드·테스트 완료, AWS Saved Plan 생성·승인·Apply와 실상태 검증
11. 예정: 최초 관리자 Bootstrap과 관리자 등록 차단
