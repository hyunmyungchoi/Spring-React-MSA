...
이 디렉터리는 `spring-react-msa` 학습 환경의 현재 AWS 인프라를 관리한다. 기본 리전은 `ap-northeast-2`다. Foundation 기준선, Backend ECR/GitHub OIDC, Private App 송신, RDS/Secrets Data Layer, ECS Compute, Database Bootstrap·Flyway Migration, Application·Frontend·Public Domain Foundation과 Runtime 수명주기를 Apply했다. 2026-07-22 관측성 Runtime ON 검증으로 현재 ECS Service·Task·Container Health 8/8, ASG `1/1/2`, EC2·Container Instance 1대, Public ALB Target 2/2 `healthy`, Valkey·RDS `available`, Container Insights `enabled`, Runtime Alarm 29개 `OK`다. Frontend S3 6개·CloudFront 2개와 배포 IAM도 유지하며 정적·API curl, OAuth·Session·WebSocket·SNS 전달을 검증했다. 동일 Runtime ON 입력 Terraform 재계획은 `No changes`다. 비용 종료 OFF는 새 Saved Plan의 별도 승인 단계다. 후속 Runtime의 승인된 목표와 미구현 경계는 [AWS Learning Runtime 결정](../../../docs/aws-migration/07-learning-runtime-design.md)을 따른다.
...
> 2026-07-22 갱신: 아래 8번의 SNS Email 확인·실알림 검증까지 완료했다. Runtime ON 관측성 검증은 `Delivered 2`·`Failed 0`과 `No changes`로 끝났으며, 다음 승인 단계는 Runtime OFF 비용 종료다. 이후 알림 전용 Watchdog과 최초 관리자 Bootstrap, Backup Restore 순서로 진행한다.
...
9. 다음: 별도 승인된 Runtime OFF Saved Plan 적용과 RDS 정지로 비용 리소스 종료
10. 예정: 알림 전용 Watchdog과 최초 관리자 Bootstrap·관리자 등록 차단
11. 예정: Backup Restore와 전체 Smoke Test
